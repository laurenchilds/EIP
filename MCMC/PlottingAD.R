...
par(mfrow = c(2,2))
# Oocysts
plot(N_obs$timeA,N_obs$OA,pch=16,xlab="time",ylab="oocysts",ylim=c(0,300))
points(N_obs$timeD,N_obs$OD,pch=16)
...
points(N_obs$timeA,N_obs$OA,pch=16,ylim=c(0, 150))
points(N_obs$timeD,N_obs$OD,pch=16,ylim=c(0, 150))


# Sporozoite numbers
plot(N_obs$timeA,floor(10^N_obs$SA-1),pch=16,xlab="time",ylab="sporozoites")
points(N_obs$timeD,floor(10^N_obs$SD-1),pch=16,xlab="time",ylab="sporozoites")
for(i in seq_along(post_traj$sims)) {
  DATA1 <- as.data.frame(post_traj$sims[i])
  lines(DATA1[,ncol(DATA1)] ~ DATA1[,1],col = "red")
}
lines(out2[,1],out2[,ncol(out2)],lwd=1,col="blue")
points(N_obs$timeA,floor(10^N_obs$SA-1),pch=16)
points(N_obs$timeD,floor(10^N_obs$SD-1),pch=16)


# Sporozoite score
plot(N_obs$timeA,N_obs$SA,pch=16,xlab="time",ylab="sporo score")
points(N_obs$timeD,N_obs$SD,pch=16)
for(i in seq_along(post_traj$sims)) {
  DATA1 <- as.data.frame(post_traj$sims[i])
  lines(log10(DATA1[,ncol(DATA1)]) ~ DATA1[,1],col = "red")
}
lines(out2[,1],log10(out2[,ncol(out2)]),lwd=1,col="blue")
points(N_obs$timeA,N_obs$SA,pch=16)
points(N_obs$timeD,N_obs$SD,pch=16)
