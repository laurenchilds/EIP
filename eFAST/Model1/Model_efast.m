...
Y1(NS,1,2,length(pmin),NR)=0;  % pre-allocation
...
for i=1:k % i=# of replications (or blocks)
    ...
    for L=1:NR
        ...
        for run_num=1:NS
            ...
            Y(run_num,:,:,i,L)=[y(time_points+1,1) sum(y(time_points+1,2:N+1),2) ...
                y(time_points+1,N+2) y(time_points+1,N+3) sum(y(time_points+1,2:N+2),2)];
            ...
            X1 = 0:.001:42;
            Ycdf(:) = gamcdf(X1,N,1/(k1*N));
            [minval,median_ix] = min(abs(Ycdf-0.5));
            Tstar = X1(median_ix);
            EIP = 1/(muE+sigmaE)+1/k1;
            EIP2 = 1/(muE+sigmaE)+Tstar;
            Y1(run_num,1,1,i,L)=EIP;
            Y1(run_num,1,2,i,L)=EIP2;
        end %run_num=1:NS
    end % L=1:NR
end % i=1:k
